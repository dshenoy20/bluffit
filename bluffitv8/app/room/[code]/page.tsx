...
export default function RoomPage() {
  ...
  useHostClaim(code, inRoom && !expired ? uid : undefined, state, now);
  ...
  if (expired) {
    return (
      <Shell>
        <Notice
          title="Game expired"
          body="This game has expired due to inactivity."
          onHome={() => router.push("/")}
  ...
  // Every member must pick a unique character before entering the lobby
  // (also shown when changing avatars from the lobby).
  if (!room.avatars?.[user.uid] || (changingAvatar && room.phase === "LOBBY")) {
    return (
      <Shell>
        <AvatarPicker
          room={room}
          uid={user.uid}
          onDone={room.avatars?.[user.uid] ? () => setChangingAvatar(false) : undefined}
        />
      </Shell>
    );
  }
