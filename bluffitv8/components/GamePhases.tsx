...
/* ---------------- REVEAL: personal result (no points shown), host advances ---------------- */
...
export function RevealPhase({
  ...
  // name -> uid lookup so we can show WHO did what, with their avatar.
  const uidByName = Object.fromEntries(
    Object.entries(room.roster).map(([id, n]) => [n, id])
  );
  const avatarOf = (name: string | null) =>
    name ? room.avatars?.[uidByName[name] ?? ""] : undefined;
  const myFooledVoters =
    room.revealOrder.find((i) => i.optionId === uid)?.voterNames ?? [];
    ...
    <Card>
      <div className="flex flex-col gap-5">
        ...
        <div className="anim-reveal flex flex-col gap-3">
          ...
          >
            {/* who fooled you, face first */}
            {result.kind === "fooled" && (
              <div className="mb-3 flex justify-center">
                <PlayerAvatar
                  avatar={avatarOf(result.fooledBy)}
                  name={result.fooledBy ?? "?"}
                  size="xl"
                />
              </div>
            )}
          ...
          {result.bluffVotes > 0 ? (
            <div className="anim-pop-in delay-300ms rounded-2xl bg-amber-400/10 px-4 py-3.5 text-center ring-1 ring-amber-400/30">
              <p className="font-bold text-amber-300">
                😎 You fooled{" "}
                {result.bluffVotes === 1
                  ? `${myFooledVoters[0] ?? "1 player"}!`
                  : `${result.bluffVotes} players!`}
              </p>
              <div className="mt-2 flex flex-wrap items-center justify-center gap-1.5">
                {myFooledVoters.map((n, i) => (
                  <span key={i} className="flex items-center gap-1 rounded-full bg-white/10 py-0.5 pl-0.5 pr-2">
                    <PlayerAvatar avatar={avatarOf(n)} name={n} size="xs" />
        ...
        {isHost ? (
          <Button
            ...
            onClick={async () => {
              setAdvancing(true);
              await advanceFromReveal(room.roomCode).catch(() => {});
              setAdvancing(false);
            }}
          >
            Show Leaderboard
          ...
          <WaitingNote>Waiting for the host</WaitingNote>
        )}
      </div>
    </Card>
  );
