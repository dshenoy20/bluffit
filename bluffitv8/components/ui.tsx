...
export function Page({ children }: { children: ReactNode }) {
  return (
    <main
      className="stage flex min-h-dvh flex-col items-center justify-center gap-6 px-4 py-8 text-white"
      style={{
...
/**
 * Round indicator that is intrinsically safe at ANY round count.
 * Dots are only rendered when few enough to always fit a narrow phone next
 * to the label and timer (≤6). Larger counts get a shrinkable progress bar,
 * so 10/15/20-round games can never overflow or wrap the header.
 */
export function RoundProgress({ round, total }: { round: number; total: number }) {
  return (
    <div className="flex min-w-0 items-center gap-2.5">
      <span className="shrink-0 whitespace-nowrap text-xs font-bold uppercase tracking-widest text-slate-500">
      ...
      {total <= 6 ? (
        ...
        <div className="h-1.5 min-w-8 max-w-24 flex-1 overflow-hidden rounded-full bg-white/10">
          <div
 ...
 * Pass `startFrom` to begin the very first animation at a specific number
 * (e.g. a player's previous-round score), optionally after `delayMs`.
 */
