...
- **Scoring** (~100-point scale, two incentives): picking the real answer earns up to 100 points, ranked by vote speed (1st correct = 100, 2nd = 90, 3rd = 80 … 8th = 30, using server timestamps); every player fooled by your bluff earns you +10. The real answer never generates bluff points. One-line scoring hints appear during round 1 only.
- **Untimed answers**: the answer-submission phase has no countdown — it waits until every connected player has submitted ("2 of 4 players answered"), then advances automatically. Voting is the only timed phase (30s).
- **Round results**: after voting, each player sees a personal, point-free result screen — what happened (correct / fooled by whom, with the fooler's avatar / nobody picked your bluff) plus the real answer. It stays up until the host clicks "Show Leaderboard"; the leaderboard then shows each player's total animating up from their previous score with a "+X this round" chip, and the host clicks "Next Round".
- **Avatars**: 8 illustrated SVG characters (Cool One, Nerd, Detective, Trickster, Cowboy, Chef, Ninja, Wizard). Every player must pick one on entering a room; uniqueness per room is enforced by a transaction on the room doc (`avatars` map), so simultaneous picks can't collide. Changing (lobby only, tap your avatar) or leaving frees the previous one.
- **Host transfer**: if the host disconnects (60s presence timeout), the earliest-joined connected player claims the role via a transaction — exactly one claimer wins, and a returning old host stays a regular player.
- **Inactivity expiry**: rooms expire after 10 minutes without meaningful activity (any game-state write refreshes `lastActivity`). All clients derive the expired state from the same server timestamp and show "This game has expired due to inactivity."
