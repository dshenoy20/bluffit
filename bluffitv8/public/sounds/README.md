...
| File | Plays when |
|---|---|
| `background.mp3` | Looping background music — starts on the user's first tap/click, loops forever, pauses when muted or when the tab is hidden |
...
- `background.mp3` should be a seamless loop (1–3 min is plenty); it plays at
  low volume (`BACKGROUND_VOLUME` in `lib/sound.ts`, default 0.22) so effects
  stay audible over it.
