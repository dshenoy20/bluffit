...
export async function advanceFromVoting(code: string): Promise<void> {
  await runTransaction(getDb(), async (tx) => {
    ...
    // Per-question analytics, written once per round inside the same
    // transaction that scores it (so it can never double-count).
    const qid = getRoundQuestion(room.questionIds, round).id;
    const qDocId = typeof qid === "number" ? `legacy_${qid}` : String(qid);
    const correctPicks = votes.filter((v) => v.optionId === REAL_ANSWER_ID).length;
    const playersFooled = votes.length - correctPicks;
    const pointsGenerated = Object.values(pointsByPlayer).reduce((a, b) => a + b, 0);
    tx.set(
      doc(getDb(), "questionStats", qDocId),
      {
        timesPlayed: increment(1),
        totalVotes: increment(votes.length),
        correctPicks: increment(correctPicks),
        playersFooled: increment(playersFooled),
        pointsGenerated: increment(pointsGenerated),
      },
      { merge: true }
    );
  });
...
export async function continueFromScoreboard(code: string): Promise<void> {
  await runTransaction(getDb(), async (tx) => {
    ...
    if (room.currentRound < getTotalRounds(room)) {
      tx.update(roomRef(code), {
        phase: "ANSWER",
