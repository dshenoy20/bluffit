...
- **Question pool**: `data/questions.json` holds 30 bluff-optimized questions across 12 categories. Each game (and each Play Again) randomly picks 5 unique ids and stores them on the room doc as `questionIds`, so every client sees the same set and rematches get fresh questions. To expand the pool, just append to the JSON with a new unique `id`. Rooms created before this field existed fall back to the first 5 questions.
...
data/questions.json     Pool of 30 questions (5 picked at random per game)
