  ...
  return (
    ...
    >
      ...
      <span
        ...
        aria-hidden
      />
      <span className="relative flex items-center gap-3 px-3.5 py-2.5">
...
function Leaderboard({ players, final }: { players: Player[]; final?: boolean }) {
  const ranked = rankPlayers(players);
  const maxScore = ranked[0]?.score ?? 0;
  return (
    <ul className="flex flex-col gap-2">
      {ranked.map((p, i) => (
        <ScoreRow key={p.id} p={p} index={i} maxScore={maxScore} final={final} />
      ))}
    </ul>
  );
}
