...
export function usePhaseDriver(
  ...
  useEffect(() => {
    ...
    // REVEAL is a short, timed personal-result screen: auto-advance to the
    // scoreboard when its deadline passes. (Legacy rooms scored by the old
    // build have no deadline in REVEAL — advance them right away.)
    if (room.phase === "REVEAL") {
      const deadline = room.phaseEndsAt ?? 0;
      const wait = isHost ? 300 : 5000;
      if (now > deadline + wait) void tryAdvance(advanceFromReveal);
      return;
    }
