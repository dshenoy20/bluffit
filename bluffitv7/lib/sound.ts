...
const BACKGROUND_FILE = "/sounds/background.mp3";
const BACKGROUND_VOLUME = 0.22;

let bgAudio: HTMLAudioElement | null = null;
let bgWanted = false; // user has interacted and music should be running

function bgPlay(): void {
  if (!bgWanted || !isSoundEnabled()) return;
  try {
    if (!bgAudio) {
      bgAudio = new Audio(BACKGROUND_FILE);
      bgAudio.loop = true;
...
/**
 * Start the looping background track. Must be called from (or after) a user
 * gesture, so browsers allow playback. Idempotent — safe to call repeatedly.
 */
...
/** Keep background music in sync with the mute toggle. */
subscribeSoundInternal(() => {
  if (isSoundEnabled()) bgPlay();
  else bgPause();
});
...
if (typeof document !== "undefined") {
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") bgPause();
    else bgPlay();
  });
}

function subscribeSoundInternal(cb: () => void): void {
  listeners.add(cb);
}
