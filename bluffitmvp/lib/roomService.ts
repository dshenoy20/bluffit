...
export async function submitAnswer(code: string, uid: string, text: string): Promise<void> {
  await runTransaction(getDb(), async (tx) => {
    ...
    const real = getQuestion(round).answer;
...
export async function advanceFromAnswer(code: string): Promise<void> {
  await runTransaction(getDb(), async (tx) => {
    ...
    const options = buildVotingOptions(submissions, getQuestion(round).answer);
    tx.update(roomRef(code), {
      phase: "VOTING",
...
/** REVEAL: next answer, or SCOREBOARD after the last one. Host-controlled. */
export async function advanceReveal(code: string): Promise<void> {
  await runTransaction(getDb(), async (tx) => {
    const room = await getRoomTx(tx, roomRef(code));
    if (room.phase !== "REVEAL") return;
    if (room.revealIndex < room.revealOrder.length - 1) {
      tx.update(roomRef(code), { revealIndex: room.revealIndex + 1, ...touch });
    } else {
      tx.update(roomRef(code), { phase: "SCOREBOARD", ...touch });
    }
  });
}
